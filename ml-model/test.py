  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://127.0.0.1:5000/predict", {
       ...
       // Store both the probability and the phishing determination
      setResult(response.data.phishing_probability);
      setIsPhishing(response.data.is_phishing);

    } catch (err) {
      setError("Error predicting phishing probability.");
    }
  };

  // Determine result color based on phishing status
  const getResultClass = () => {
    if (isPhishing === "Yes") return "result phishing";
    if (isPhishing === "No") return "result safe";
    return "result";
  };
