...
def extract_enhanced_features(url):
    try:
        ...
        directory_lengths = [len(x) for x in parsed.path.split('/') if x]
        directory_length_mean = float(np.mean(directory_lengths)) if directory_lengths else 0.0
        ...
        features = [float(x) if isinstance(x, (int, float)) and not np.isnan(x) else 0.0 for x in features]
        return features
    except:
        return {k: 0 for k in [
            'length_url', 'length_hostname', 'length_path', 'length_query', 'length_fragment',
